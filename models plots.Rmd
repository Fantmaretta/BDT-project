...
```{r}
df <- read.csv("/home/veror/Downloads/data_plots/compare_pioggia.csv")
...
```{r}
# MODEL WITH PREDICTION ABOUT INTENSITY
...
df <- read.csv("/home/veror/Desktop/Uni/Bid Data Technology/R BDT project/accuracy_12.csv")
View(df)
df$id_prec_prob <- as.factor(df$id_prec_prob)
df$id_prec_int <- as.factor(df$id_prec_prob)
df$id_previsione_giorno <- as.factor(df$id_)



# bagging
set.seed(1)
# set number of predictors (all)
n_pred <- 3
# bagging on the training portion of hit
bag <- randomForest(id_prec_prob ~ id_prec_int + temp_max + temp_min, data = df, mtry=n_pred, importance=TRUE)
bag
#hit_bag$importance # predictors importance
# plot importance
varImpPlot(bag)
```
